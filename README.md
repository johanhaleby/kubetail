...
Install the script using homebrew to dynamically display the pods names on `$ kubetail <tab>`. Alternatively install the [completion script](completion/) (bash/zsh/fish) manually. For Ubuntu, copy the scripts to `/etc/bash_completion.d/` and restart your terminal). For zsh on Mac copy the script to `/usr/local/share/zsh/site-functions/_kubetail`.
